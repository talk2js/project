define([ 
	...
	OpenLayers.Layer.Tianditu = OpenLayers.Class(OpenLayers.Layer.Grid, {
		...
		getURL : function(bounds) {
			...
			var y_num = this.topTileFromY < this.topTileToY ? Math
					.round((bounds.bottom - this.topTileFromY) / coef) : Math
					.round((this.topTileFromY - bounds.top) / coef);
		...
		clone : function(obj) {
			if (obj == null) {
				obj = new OpenLayers.Layer.Tianditu(this.name, this.url,
						this.options);
			}
			obj = OpenLayers.Layer.Grid.prototype.clone.apply(this,
					[ obj ]);
			return obj;
		},
	...
	return OpenLayers.Layer.Tianditu;
