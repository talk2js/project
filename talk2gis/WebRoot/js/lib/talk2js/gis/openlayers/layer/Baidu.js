define([ 
	...
	OpenLayers.Layer.Baidu = OpenLayers.Class(OpenLayers.Layer.XYZ, {
		...
		initialize: function (options) {
			...
			// this block steps through translating the values the server layer JSON
			// capabilities object into values that we can use. This is also a
			// helpful
		...
		getContainingTileCoords: function (point, res) {
			...
			// Math.max(Math.floor((point.x - this.tileOrigin.lon) /
			// (this.tileSize.w * res)), 0),
			// Math.max(Math.floor((this.tileOrigin.lat - point.y) /
			// (this.tileSize.h * res)), 0)
			// );
		...
		getURL: function (bounds) {
			...
			// var originTileY = (this.tileOrigin.lat - (res * this.tileSize.h /
			// 2));
			...
			// var x = (Math.round(Math.abs((center.lon - originTileX) / (res *
			// this.tileSize.w))));
			// //var y = (Math.round(Math.abs((originTileY - center.lat) / (res *
			// this.tileSize.h))));
			// var y = (Math.round(Math.abs((center.lat - originTileY) / (res *
			// this.tileSize.h))));
			...
			// this prevents us getting pink tiles (non-existant tiles)
			if (this.lods) {
		...
		clone : function(obj) {
			if (obj == null) {
				obj = new OpenLayers.Layer.Baidu(this.name, this.url,
						this.options);
			}
