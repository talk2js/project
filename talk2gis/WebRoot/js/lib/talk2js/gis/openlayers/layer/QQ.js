define([ 
	...
	OpenLayers.Layer.QQ = OpenLayers.Class(OpenLayers.Layer.TileCache, {
		...
		getURL : function(bounds) {
			...
			var scope = new Array(0, 0, 0, 0, 0, 3, 0, 3, 0, 3, 0, 3, 0, 7, 0,
					7, 0, 15, 0, 15, 0, 31, 0, 31, 0, 63, 4, 59, 0, 127, 12,
					115, 0, 225, 28, 227, 356, 455, 150, 259, 720, 899, 320,
					469, 1440, 1799, 650, 929, 2880, 3589, 1200, 2069, 5760,
					7179, 2550, 3709, 11520, 14349, 5100, 7999, 23060, 28689,
					10710, 15429, 46120, 57369, 20290, 29849, 89990, 124729,
			...
			if (tileX >= i && tileX <= j && tileY >= l && tileY <= scope) {
				...
				var tileNo = tileZ + "/" + Math.floor(tileX / 16) + "/"
						+ Math.floor(tileY / 16) + "/" + tileX + "_" + tileY
						+ imgformat;
			}
		...
		clone : function(obj) {
			if (obj == null) {
				obj = new OpenLayers.Layer.QQ(this.name, this.url, this.options);
			}
